...
module.exports = function (port, theme, silent, dir, resumeFilename) {
  ...
  // make glob pattern to be resumeFilename + all dependencies files
  const otherPaths = [
    "*(",
    "dependencies/**/*",
    "*.yaml",
  ...
  // join with ,
  const globPattern = otherPaths.join('|')
  console.log("watching glob pattern: ", globPattern)

  bs.watch(globPattern).on('change', () => {
